-- <pre> A3042B-4 Telemetry Control Box (TCB)Firmware. 

-- V1.1, 15-JUL-26: Based upon P3042BB V7.1. Add new inputs and
-- outputs, remove unused inputs and outputs.
...
entity main is 
	port (
		...
		cont_addr : in std_logic_vector(7 downto 0);
		CWR_in : in std_logic;
		CDS_in : in std_logic;
		...
		DC5 : in std_logic; -- Unused (DC5)
		DC6 : out std_logic; -- Unused (DC6)
		DMCK : out std_logic; -- Demodulator Clock (DC7)
		
		ON_not : out std_logic; -- On for Command Transmitters
		
		EN1_not, EN2_not : out std_logic; -- Enable Digital IO Output
		DX1, DX2 : in std_logic; -- Digital Input Data
		Q1, Q2 : out std_logic; -- Digital Output
		
		HDW : in std_logic; -- Hardware Identifier Bit
		
		TP1, TP2, TP3, TP4 : out std_logic; -- Test Points
		
		RB2, RB5 : in std_logic -- Spare EEM Connections
	);	
...
-- Reset signals.
	...
	signal RESET : std_logic; -- Firmware and CPU Reset
	
-- Synchronized, delayed, and inverted inputs.
	signal RCK, MRDY, DMERR : std_logic;
...
begin
	...
	-- Reset is when we press the base board reset switch. Pressing this 
	-- switch causes a 100-ms LO on the base board !MCLR signal. The Embedded
	-- Ethernet Module (EEM) resets and reboots. The EEM asserts !RST as it
	-- reboots, which restarts its Ethernet PHY and resets the controller
	-- logic and detector modules. A Mid-Level Reset is one in which the EEM
	-- keeps running, but the controller and detector modules are reset. A
	-- mid-level reset can be initiated by the EEM writing to the LWDAQ reset
	-- location, which causes RST_BY_RELAY to be asserted, or by the CPU 
	-- writing to the MMU reset locatin, which causes RST_BY_CPU to be 
	-- asserted. A Low-Level Reset is one in which only the detector modules
	-- are reset. The low-level reset is managed by the CPU using the dmrst 
	-- location, which asserts DMRST_BY_CPU. When we see DMRST_BY_CPU, we drive
	-- the DMRST pin HI.
	...
	begin
		...
		-- We generate a mid-level reset when any of the signals
		-- !RST, RST_BY_CPU, or RST_BY_RELAY are asserted. The
		-- !RST appears in our input list as RST_not. This line
		-- is provided by the EEM. 
		if rising_edge(CK) then
			if mid_state = 0 then
				if RST_BY_CPU or RST_BY_RELAY or (RST_not = '0') then 
					mid_next_state := 1;
				else 
					mid_next_state := 0;
				end if;
			elsif mid_state = mid_rst_len then
				if RST_BY_CPU or RST_BY_RELAY then
			...
			RESET <= to_std_logic(mid_state > 1);
...
-- Random Access Memory, and Interrupt Handler. Byte ordering is big-endian. 
	...
	-- Upload and DMERR indicators are 16 and 17 respectively, negative true.
	indicators(16) <= to_std_logic(not UPLOAD);
	indicators(17) <= not DMERR;
	...
	TP4 <= RB2 or RB5 or HDW or DX1 or DX2;
	TP3 <= cont_addr(6) or cont_addr(7) or DC5 or D6;
	ON_not <= HDW;
end behavior;